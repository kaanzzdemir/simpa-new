...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var languageOption = document.getElementById('selectedLanguage');
    selectbox.value = "Türkiye (+90)"

    for (var i = 0; i < countries.length; i++) {
        var option = document.createElement("option");
        option.text = countries[i].name + " (" + countries[i].dial_code + ")";
        option.value = countries[i].code;
        // Türkiye standart olarak seçili olsun
        if (countries[i].code === "TR") {
            option.selected = true;
            languageOption.text = "Türkçe";
        }
        selectbox.add(option);
    }

    // İlk seçim değiştikçe çağrılacak işlev
    selectbox.addEventListener('change', function () {
        // İlk seçilen değeri al
        var selectedCountry = this.value;

        // Şehirleri temizle
        languageOption.text = '';

        // Seçilen ülkeye göre dil seçeneklerini ekle
        if (selectedCountry === "TR") {
            languageOption.text = "Türkçe";
        } else if (selectedCountry === 'AE' || selectedCountry === 'SA' || selectedCountry === 'SY') {
            languageOption.text = "Arapça";
        }
        else {
            languageOption.text = "İngilizce";
        }
        // Diğer ülkeler için gerekli işlemleri yapabilirsiniz.
    });
    var selectLangButton = document.getElementById("selectLanguageButton");
    var selectLang = document.getElementById("selectedLanguage");
    ...
    selectLangButton.addEventListener('click', function () {
        if (selectLang.innerHTML == "İngilizce") {
            trButton.click();
        }
        else if(selectLang.innerHTML == "Türkçe"){
            enButton.click();
        }
        else if (selectLang.innerHTML == "Arapça") {
            arButton.click();
        }
    });
});
